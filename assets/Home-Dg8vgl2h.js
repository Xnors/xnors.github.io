...
<slot name="end"></slot>
`;class bt extends b({style:vt,template:xt,props:ft}){}bt.define(gt);const yt="s-appbar",wt=`
:host{
  display: flex;
  align-items: center;
  position: relative;
...
<slot name="end"></slot>
`;class kt extends b({style:wt,template:$t,setup(t){const e=t.querySelector(".view");te.CSSContainer||new ResizeObserver(()=>{e.classList.toggle("s-laptop",this.offsetWidth<=1024),e.classList.toggle("s-tablet",this.offsetWidth<=768)}).observe(this)}}){}kt.define(yt);const Et="s-avatar",Ct=w({$src:""}),St=`
:host{
  display: inline-flex;
...
`;class At extends b({style:St,template:qt,props:Ct,setup(t){const e=document.createElement("img");return{src:o=>{e.src=o,e.onload=()=>{this.dispatchEvent(new Event("load")),t.insertBefore(e,t.children[0])},e.onerror=()=>{this.dispatchEvent(new ErrorEvent("error")),e.isConnected&&t.removeChild(e)}}}}}){}At.define(Et);const Tt="s-badge",Ft=`
:host{
  display: inline-flex;
  justify-content: center;
  align-items: center;
...
`;class zt extends b({style:Ft,template:Mt}){}zt.define(Tt);const z=s=>{const t=s.trim().match(/^([\d\.]+)(s|ms)$/);if(!t)throw new Error;const e=Number(t[1]);return t[2]==="s"?e*1e3:e},Lt="s-bottom-sheet",Bt=w({showed:!1,disabledGesture:!1}),Dt=oe({show:CustomEvent,showed:Event,close:CustomEvent,closed:Event}),Nt=`
:host{
...
`;class Wt extends b({style:Qt,template:Vt,props:Ot}){}Wt.define(It);const Yt="s-card",Jt=w({type:["elevated","filled","outlined"],clickable:!1}),Gt=`
:host{
  display: inline-block;
  vertical-align: middle;
  border-radius: 12px;
...
`,no="<slot></slot>";class qe extends b({style:ro,template:no,props:so,setup(){return{selected:()=>{this.parentNode instanceof Se&&this.dispatchEvent(new Event(`${X}:update`,{bubbles:!0}))}}}}){}Se.define(X);qe.define(oo);const io="s-checkbox",ao=w({disabled:!1,checked:!1,indeterminate:!1}),lo=`
:host{
  display: inline-flex;
  align-items: center;
  vertical-align: middle;
...
<slot></slot>
<s-ripple class="ripple" attached="true" part="ripple"></s-ripple>
`;class po extends b({style:lo,template:co,props:ao,setup(){this.addEventListener("click",()=>{this.indeterminate&&(this.indeterminate=!1),this.checked=!this.checked,this.dispatchEvent(new Event("change"))})}}){}po.define(io);const ho="s-chip",uo=w({type:["filled","outlined"],$value:"",checked:!1,disabled:!1,clickable:!1}),mo=`
:host{
  display: inline-flex;
  align-items: center;
  vertical-align: middle;
...
<slot name="action"></slot>
<s-ripple class="ripple" attached="true" part="ripple"></s-ripple>
`;class fo extends b({style:mo,template:go,props:uo,setup(t){const e=t.querySelector("slot[name=action]");e.onclick=o=>o.stopPropagation(),e.onpointerdown=o=>o.stopPropagation(),this.addEventListener("click",()=>{this.clickable&&(this.checked=!this.checked,this.dispatchEvent(new Event("change")))})}}){}fo.define(ho);const vo="s-circular-progress",xo=w({indeterminate:!1,animated:!1,$max:100,$value:0}),bo=`
:host{
  display: inline-block;
  vertical-align: middle;
  position: relative;
...
`;class wo extends b({style:bo,template:yo,props:xo,setup(t){const e=t.querySelector(".known .track"),o=t.querySelector(".known .indicator"),i=()=>{const n=Math.min(this.value,this.max)/this.max*100,l=P-P*(n/100),a=n/100*360;e.style.strokeDashoffset=`${n===0?0:Math.min(P+16-l,P)}px`,e.setAttribute("transform",`rotate(${a+20}, ${q/2}, ${q/2})`),o.style.strokeDashoffset=`${l}px`};return{max:i,value:i}}}){}wo.define(vo);const K=(s,t="yyyy-MM-dd")=>{typeof s=="string"&&(s=new Date(s));const e=s.getFullYear(),o=s.getMonth()+1,i=s.getDate();return t.replace("yyyy",e.toString()).replace("MM",o.toString().padStart(2,"0")).replace("dd",i.toString().padStart(2,"0"))};class $o{constructor(t){S(this,"list");S(this,"locale",navigator.language);S(this,"updates",new Map);this.list=t}getItem(t){if(t=t||this.locale,t in this.list)return this.list[t];const[e]=t.split("-");return e in this.list?this.list[e]:this.list.zh}addItem(t,e){if(this.list[t])throw new Error(`Locale ${t} already exists`);this.list[t]=e}setLocale(t){this.locale=t??navigator.language,this.updates.forEach(e=>e())}}const B=new $o({});B.list={zh:{display:s=>`${s.getFullYear()}年${s.getMonth()+1}月${s.getDate()}日 星期${B.list.zh.displayWeeks[s.getDay()]}`,displayMonth:s=>`${s.getFullYear()}年`,displayWeeks:["日","一","二","三","四","五","六"]},en:{display:s=>`${["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][s.getDay()]}, Jan ${s.getDate()}`,displayMonth:s=>`${["January","February","March","April","May","June","July","August","September","October","November","December"][s.getMonth()]} ${s.getFullYear()}`,displayWeeks:["S","M","T","W","T","F","S"]}};const ko="s-date",Eo=w({$value:"",$locale:"",$max:"2099-12-31",$min:"1900-01-01"}),Co=`
:host{
  display: inline-block;
  vertical-align: middle;
  border-radius: 8px;
...
`,_={years:[],weeks:[],days:[]};for(let s=0;s<200;s++)s<7&&_.weeks.push('<div class="item"></div>'),s<31&&_.days.push(`<div class="item"><s-ripple class="icon-button">${s+1}</s-ripple></div>`),_.years.push('<s-ripple class="button item"></s-ripple>');const So=`
...
`,Po="<slot></slot>";class Ho extends b({style:No,template:Po}){}Ho.define(Do);const jo="s-dialog",Zo=w({showed:!1,size:["standard","full"]}),_o=oe({show:CustomEvent,showed:Event,close:CustomEvent,closed:Event}),Uo=`
:host{
  display: inline-block;
  vertical-align: middle;
}
...
`,Wo="<slot></slot>";class Yo extends b({style:Vo,template:Wo}){}Yo.define(Qo);const Jo="s-drawer",Go=`
:host{
  display: flex;
  height: 100%;
  overflow: hidden;
...
<slot></slot>`;class os extends b({style:es,template:ts}){}os.define(Ro);const ss="s-fab",rs=w({hidden:!1,disabled:!1}),ns=`
:host{
  display: inline-flex;
  vertical-align: middle;
  justify-content: center;
...
`;class as extends b({style:ns,template:is,props:rs}){}as.define(ss);const ls="s-icon-button",cs=w({disabled:!1,type:["standard","filled","filled-tonal","outlined"]}),ds=`
:host{
  display: inline-flex;
  vertical-align: middle;
  justify-content: center;
...
`,fs="<slot></slot>";class vs extends b({style:gs,template:fs,props:ms,setup(t){const e=t.querySelector("slot"),o=document.createElement("img"),i=(n=I.none,l="")=>`<svg viewBox="0 -960 960 960"><path d="${n}" transform="${l}"></path></svg>`;return{name:n=>{const l=I[n];if(typeof l=="string")return e.innerHTML=i(l);const a=l.name;typeof I[a]=="string"&&(e.innerHTML=i(I[a],`rotate(${l.angle} 480 -480)`))},src:async()=>{try{const n=new URL(this.src,location.href);if(n.pathname.endsWith(".svg")){const a=await(await fetch(n.href)).text(),c=document.createElement("template");c.innerHTML=a;const d=c.content.childNodes[0];if(!(d instanceof SVGElement))throw new Error("Invalid SVG");e.innerHTML="",e.appendChild(d),this.dispatchEvent(new Event("load"))}else o.src=n.href,o.onload=()=>{e.innerHTML="",e.appendChild(o),this.dispatchEvent(new Event("load"))},o.onerror=()=>this.dispatchEvent(new ErrorEvent("error"))}catch{this.dispatchEvent(new ErrorEvent("error"))}}}}}){}vs.define(us);const xs="s-linear-progress",bs=w({indeterminate:!1,animated:!1,$max:100,$value:0}),ys=`
:host{
  display: block;
  height: 4px;
  color: var(--s-color-primary, #006782);
...
`;class $s extends b({style:ys,template:ws,props:bs,setup(t){const e=t.querySelector(".known>.track"),o=t.querySelector(".known>.indicator"),i=()=>{const n=Math.min(this.value,this.max)/this.max*100;e.style.transform=`translateX(calc(${n}% + ${n===0?0:4}px))`,o.style.transform=`translateX(${n-100}%)`};return{max:i,value:i}}}){}$s.define(xs);const ks="s-menu",Es=`
:host{
  display: flex;
  flex-direction: column;
  font-size: .875rem;
...
<slot></slot>
`;class Ss extends b({style:Es,template:Cs}){}const qs="s-menu-item",Te=w({checked:!1,folded:!0}),As=`
:host{
  display: flex;
  flex-direction: column;
...
`;class Fs extends b({style:As,template:Ts,props:Te,setup(t){const e=t.querySelector(".container"),o=t.querySelector(".fold"),i=t.querySelector("slot[name=menu]");return o.onclick=n=>n.stopPropagation(),i.onslotchange=()=>e.classList[i.assignedElements().length>0?"add":"remove"]("show-menu"),e.onclick=()=>{e.classList.contains("show-menu")&&(this.folded=!this.folded)},{folded:n=>o.folded=n}}}){}Ss.define(ks);Fs.define(qs);const ie="s-navigation",Ms=w({mode:["bottom","rail"],$value:""}),zs=`
:host{
  display: flex;
  justify-content: center;
  align-items: center;
...
<slot id="slot"></slot>
<slot name="end"></slot>
`;class ae extends b({style:zs,template:Ls,props:Ms,setup(t){const e=t.querySelector("#slot"),o=new Q({context:this,class:Fe,slot:e});return{expose:{get options(){return o.list},get selectedIndex(){return o.selectedIndex}},value:{get:()=>o.value,set:i=>o.value=i}}}}){}const Bs="s-navigation-item",Ds=w({selected:!1,$value:""}),Ns=`
:host{
  display: flex;
  align-items: center;
  justify-content: center;
...
<slot name="text"></slot>
`;class Fe extends b({style:Ns,template:Ps,props:Ds,setup(){return this.addEventListener("click",()=>{this.selected||this.parentNode instanceof ae&&this.dispatchEvent(new Event(`${ie}:select`,{bubbles:!0}))}),{selected:()=>{this.parentNode instanceof ae&&this.dispatchEvent(new Event(`${ie}:update`,{bubbles:!0}))}}}}){}ae.define(ie);Fe.define(Bs);const Hs="s-field",js=w({focused:!1,fixed:!0}),Zs=`
:host{
  display: inline-block;
  vertical-align: middle;
  font-size: .875rem;
...
`;class Us extends b({style:Zs,template:_s,props:js}){}Us.define(Hs);const Is="s-fold",Os=w({folded:!1}),Qs=`
:host{
  display: block;
}
.container{
...
`;class Ws extends b({style:Qs,template:Vs,props:Os,setup(t){t.querySelector("slot[name=trigger]").onclick=()=>this.folded=!this.folded}}){}Ws.define(Is);const Ys="s-page",Js=w({theme:["light","auto","dark"]}),Gs=`
:host{
  display: flow-root;
  height: 100%;
...
`;class Me extends b({style:cr,template:dr,props:lr,setup(){return this.addEventListener("click",()=>{this.selected||this.parentNode instanceof de&&this.dispatchEvent(new Event(`${ce}:select`,{bubbles:!0}))}),{selected:()=>{this.parentNode instanceof de&&this.dispatchEvent(new Event(`${ce}:update`,{bubbles:!0}))}}}}){}de.define(ce);Me.define(ar);const pr="s-popup",hr=w({align:["center","left","right"]}),ur=oe({show:Event,showed:Event,closed:Event}),mr=`
:host{
  display: inline-block;
  vertical-align: middle;
  text-align: left;
...
`;class R extends b({style:br,template:yr,props:xr,setup(t){const e=t.querySelector(".popup"),o=t.querySelector("slot[name=trigger]");return o.onclick=i=>{i.stopPropagation(),e.show()},this.addEventListener(`${fe}:click`,i=>{i.stopPropagation(),e.close()}),e.onclose=i=>{const n=i.target;if(n.parentNode instanceof ShadowRoot){const l=n.parentNode.host;l.parentNode instanceof R&&l.parentNode.close()}},{onMounted:()=>{this.parentNode instanceof R&&e.setAttribute("align","right")},expose:{get show(){return e.show},get toggle(){return e.toggle},get close(){return e.close}}}}}){}const wr="s-popup-menu-item",$r=`
:host{
  display: flex;
  align-items: center;
  height: 40px;
...
`;class Er extends b({style:$r,template:kr,setup(){this.addEventListener("click",()=>this.dispatchEvent(new Event(`${fe}:click`,{bubbles:!0})))}}){}R.define(fe);Er.define(wr);const Cr="s-radio-button",Sr=w({disabled:!1,checked:!1,name:"",$value:""}),qr=`
:host{
  display: inline-flex;
  vertical-align: middle;
  align-items: center;
...
<slot></slot>
<s-ripple class="ripple" attached="true" part="ripple"></s-ripple>
`;class Tr extends b({style:qr,template:Ar,props:Sr,setup(){this.addEventListener("click",()=>{this.checked=!0,this.name&&document.querySelectorAll(`${this.tagName}[name='${this.name}']`).forEach(t=>{t!==this&&(t.checked=!1)}),this.dispatchEvent(new Event("change"))})}}){}Tr.define(Cr);const Fr="s-rate",O=w({readOnly:!1,$max:10,$min:0,$value:5,$step:1}),Mr=`
:host{
  display: inline-flex;
  vertical-align: middle;
  overflow: hidden;
...
<input
  ...
  value="${O.value}"
/>
`;class Lr extends b({style:Mr,template:zr,props:O,setup(t){const e=t.querySelector(".indicator"),o=t.querySelector("input"),i=()=>{const l=(Number(o.value)-this.min)*100/this.max-this.min;e.style.width=`${l}%`};return o.onchange=()=>this.dispatchEvent(new Event("change")),o.oninput=()=>{this.value=Number(o.value),this.dispatchEvent(new Event("input"))},{max:n=>{o.max=String(n),i()},min:n=>{o.min=String(n),i()},step:n=>{o.step=String(n),i()},value:n=>{o.value=String(n),i()}}}}){}Lr.define(Fr);const Br="s-search",Dr=w({$placeholder:"",disabled:!1,$value:"",$maxLength:-1,readOnly:!1}),Nr=`
:host{
  display: inline-flex;
  vertical-align: middle;
  min-height: 40px;
...
`;class Hr extends b({style:Nr,template:Pr,props:Dr,setup(t){const e=t.querySelector("input"),o=t.querySelector(".clear"),i=t.querySelector("[name=dropdown]"),n=()=>o.style.display=e.value===""?"none":"flex";return i.onmousedown=l=>l.preventDefault(),e.onchange=()=>this.dispatchEvent(new Event("change")),o.onclick=()=>{e.value="",n(),this.dispatchEvent(new Event("input")),this.dispatchEvent(new Event("change"))},e.oninput=n,{expose:{get native(){return e}},value:{get:()=>e.value,set:l=>{e.value=l,n()}},placeholder:l=>e.placeholder=l,maxLength:l=>e.maxLength=l,readOnly:l=>e.readOnly=l}}}){}Hr.define(Br);const pe="s-segmented-button",jr=w({$value:"",mode:["auto","fixed"]}),Zr=`
:host{
  display: inline-flex;
  vertical-align: middle;
  align-items: center;
...
<slot name="end"></slot>
<s-ripple attached="true" part="ripple"></s-ripple>
`;class ze extends b({style:Or,template:Qr,props:Ir,setup(){return this.addEventListener("click",()=>{!(this.parentNode instanceof he)||this.selected||this.selectable&&this.dispatchEvent(new Event(`${pe}:select`,{bubbles:!0}))}),{selected:()=>{this.parentNode instanceof he&&this.dispatchEvent(new CustomEvent(`${pe}:update`,{bubbles:!0,detail:{}}))}}}}){}he.define(pe);ze.define(Ur);const Vr="s-skeleton",Wr=`
:host{
  display: block;
  height: 16px;
...
`;class Yr extends b({style:Wr}){}Yr.define(Vr);const Jr="s-slider",j=w({disabled:!1,$labeled:!1,$max:100,$min:0,$step:1,$value:0}),Gr=`
:host{
  display: block;
  color: var(--s-color-primary, #006782);
  height: 16px;
...
`;class dn extends b({style:ln,template:cn,props:an,setup(){this.addEventListener("click",()=>{this.checked=!this.checked,this.dispatchEvent(new Event("change"))})}}){}dn.define(nn);const ue="s-tab",pn=w({mode:["scrollable","fixed"],$value:""}),hn=`
:host{
  display: flex;
  justify-content: flex-start;
  position: relative;
...
`,yn=`
<slot></slot>
`;class wn extends b({style:bn,template:yn}){}const $n="s-thead",kn=`
:host{
...
`,En="<slot></slot>";class Cn extends b({style:kn,template:En}){}const Sn="s-tbody",qn=`
:host{
...
`,An="<slot></slot>";class Tn extends b({style:qn,template:An}){}const Fn="s-tr",Mn=`
:host{
  display: table-row;
}
`,zn="<slot></slot>";class Ln extends b({style:Mn,template:zn}){}const Bn="s-th",Dn=`
:host{
  display: table-cell;
  padding: 12px 16px;
  text-transform: capitalize;
}
`,Nn="<slot></slot>";class Pn extends b({style:Dn,template:Nn}){}const Hn="s-td",jn=`
:host{
  display: table-cell;
  user-select: text;
  padding: 12px 16px;
}
`,Zn="<slot></slot>";class _n extends b({style:jn,template:Zn}){}wn.define(xn);Cn.define($n);Tn.define(Sn);Ln.define(Fn);Pn.define(Bn);_n.define(Hn);const Un="s-text-field",In=w({$label:"",$placeholder:"",disabled:!1,type:["text","number","password","multiline"],error:!1,$value:"",$maxLength:-1,readOnly:!1,multiLine:!1,countered:!1}),On=`
:host{
  display: inline-grid;
  vertical-align: middle;
  font-size: .875rem;
...
`;class Xn extends b({style:Jn,template:Gn,props:Yn,setup(t){const e=t.querySelector("slot[name=trigger]"),o=t.querySelector(".popup"),i=getComputedStyle(this);let n=!1;const l=()=>{const r=i.getPropertyValue("--s-motion-easing-standard")||"cubic-bezier(0.2, 0, 0, 1.0)",f=i.getPropertyValue("--s-motion-duration-medium4")||"400ms";return{easing:r,duration:z(f)}},a=()=>{if(!this.isConnected||this.disabled)return;if(o.style.display="block",n=!0,o.showPopover)o.showPopover();else{const y=Le(t);o.style.marginLeft=`${-y.left}px`,o.style.marginTop=`${-y.top}px`,o.style.zIndex="3"}const r=this.getBoundingClientRect(),f=4,m=o.offsetWidth,g=o.offsetHeight,p={top:0,left:0},h={middle(y){p.left=r.left-(m-r.width)/2;const v=()=>(p.top=r.top+r.height+f,p.top+g>innerHeight),x=()=>(p.top=r.top-g-f,p.top<0);p.left<0&&(p.left=r.left),p.left+m>innerWidth&&(p.left=r.left+r.width-m),y==="top"&&x()&&v(),y==="bottom"&&v()&&x()},left(){return p.left=r.left-m-f,p.top=r.top-(g-r.height)/2,p.left<0},right(){return p.left=r.left+r.width+f,p.top=r.top-(g-r.height)/2,p.left+m>innerWidth}};switch(this.align){case"bottom":case"top":h.middle(this.align);break;case"left":h.left()&&h.right();break;case"right":h.right()&&h.left();break}o.style.top=`${p.top}px`,o.style.left=`${p.left}px`,o.animate({opacity:[0,1]},l())};let c=0;const d=()=>{clearTimeout(c),n=!1,o.animate({opacity:[1,0]},l()).finished.then(()=>{n||(o.hidePopover&&o.hidePopover(),o.style.removeProperty("display"))})};e.addEventListener("touchstart",()=>{A.pointerFine.matches||(clearTimeout(c),c=setTimeout(()=>a(),600))},{passive:!0}),e.ontouchend=()=>{clearTimeout(c),d()},e.onmouseenter=()=>A.pointerFine.matches&&a(),e.onmouseleave=()=>A.pointerFine.matches&&d()}}){}Xn.define(Wn);const Kn={class:"team",id:"team"},Rn={class:"container"},ei={class:"team-grid"},ti={class:"social-links"},oi={__name:"Team",setup(s){return(t,e)=>{const o=re("s-ripple"),i=re("router-link");return M(),F("section",Kn,[u("div",Rn,[e[11]||(e[11]=u("h2",{class:"section-title fade-in"},"我们的团队",-1)),u("div",ei,[C(o,{class:"team-member fade-in"},{default:L(()=>[...e[0]||(e[0]=[u("div",{class:"member-avatar"},[u("img",{src:it,alt:"Fexcode",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"Fexcode",-1),u("p",{class:"member-role"},"爬虫数据收集与数据分析员/后端工程师",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[...e[1]||(e[1]=[u("div",{class:"member-avatar"},[u("img",{src:at,alt:"SkyMaoZi",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"SkyMaoZi",-1),u("p",{class:"member-role"},"全栈工程师",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[...e[2]||(e[2]=[u("div",{class:"member-avatar"},[u("img",{src:lt,alt:"CodeSeed",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"CodeSeed",-1),u("p",{class:"member-role"},"测试工程师",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[...e[3]||(e[3]=[u("div",{class:"member-avatar"},[u("img",{src:ct,alt:"xm",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"xm",-1),u("p",{class:"member-role"},"爬虫与数据分析员",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[e[6]||(e[6]=u("div",{class:"member-avatar"},[u("img",{src:dt,alt:"Justthewoofus",width:"100",height:"100"})],-1)),e[7]||(e[7]=u("h3",{class:"member-name"},"Justthewoofus",-1)),e[8]||(e[8]=u("p",{class:"member-role"},"底层开发工程师",-1)),u("div",ti,[C(i,{to:"justwoofus",class:"social-link"},{default:L(()=>[...e[4]||(e[4]=[G("📧",-1)])]),_:1}),C(i,{to:"justwoofus",class:"social-link"},{default:L(()=>[...e[5]||(e[5]=[G("💼",-1)])]),_:1})])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[...e[9]||(e[9]=[u("div",{class:"member-avatar"},[u("img",{src:pt,alt:"Narci",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"Narci",-1),u("p",{class:"member-role"},"后端工程师",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1}),C(o,{class:"team-member fade-in"},{default:L(()=>[...e[10]||(e[10]=[u("div",{class:"member-avatar"},[u("img",{src:ht,alt:"BaiChuanByte",width:"100",height:"100"})],-1),u("h3",{class:"member-name"},"BaiChuanByte",-1),u("p",{class:"member-role"},"室长/策划/吉祥物",-1),u("div",{class:"social-links"},[u("a",{href:"#",class:"social-link"},"📧"),u("a",{href:"#",class:"social-link"},"💼")],-1)])]),_:1})])])])}}},si={class:"contact",id:"contact"},ri={class:"container"},ni={class:"message-board fade-in"},ii={class:"message-input-container"},ai={class:"message-input-footer"},li={class:"char-count"},ci={class:"char-count"},di=["disabled"],pi={class:"messages-container"},hi={class:"message-content"},ui={class:"message-author"},mi=["onClick"],gi={__name:"Contact",setup(s){const t=Y(""),e=Y([]),o=Y(!1),i=async()=>{try{const a=await W.get("/messagebord/");a.data&&(e.value=a.data.messages||[])}catch(a){console.error("获取留言失败:",a)}},n=async()=>{var a,c;if(t.value.length<=10||t.value.length>512){alert("留言内容长度必须在10到512个字符之间");return}o.value=!0;try{(await W.post("/messagebord/",{content:t.value})).data&&(t.value="",i())}catch(d){console.error("提交留言失败:",d),alert(((c=(a=d.response)==null?void 0:a.data)==null?void 0:c.message)||"提交留言失败，请稍后重试")}finally{o.value=!1}},l=async a=>{if(confirm("确定要删除这条留言吗？"))try{(await W.delete("messagebord/",{data:{user_message_id:a}})).data&&i()}catch(c){console.error("删除留言失败:",c),alert("删除留言失败，请稍后重试")}};return ge(()=>{i()}),(a,c)=>(M(),F("section",si,[u("div",ri,[c[1]||(c[1]=u("h2",{class:"section-title fade-in"},"联系我们",-1)),u("div",ni,[u("div",ii,[_e(u("textarea",{"onUpdate:modelValue":c[0]||(c[0]=d=>t.value=d),class:"message-input",placeholder:"输入你的留言 (10-512个字符)",maxlength:"512"},null,512),[[Ue,t.value]]),u("div",ai,[u("span",li,U(t.value.length)+"/512",1),u("span",ci,U(t.value.length<10?"多写一点呗~":""),1),u("button",{onClick:n,class:"submit-btn",disabled:o.value||t.value.length<=10||t.value.length>512},U(o.value?"提交中...":"提交留言"),9,di)])]),u("div",pi,[(M(!0),F(Ee,null,Ie(e.value,d=>(M(),F("div",{key:d.id,class:Oe(["message-item",{"own-message":d.is_own}])},[u("div",hi,[u("span",ui,U(d.username)+":",1),G(" "+U(d.content),1)]),d.is_own?(M(),F("button",{key:0,onClick:r=>l(d.id),class:"delete-btn"}," × ",8,mi)):$e("",!0)],2))),128))])]),c[2]||(c[2]=u("div",{class:"contact-methods"},[u("div",{class:"contact-method fade-in"},[u("div",{class:"contact-icon",onclick:"copyToClipboard('xnors-studio@outlook.com', '邮箱已复制到剪贴板:\\nxnors-studio@outlook.com')"}," 📧 "),u("p",{class:"contact-label"},"邮箱联系")]),u("div",{class:"contact-method fade-in"},[u("div",{class:"contact-icon",onclick:"window.open('https://github.com/xnors', '_blank')"}," 💻 "),u("p",{class:"contact-label"},"GitHub")]),u("div",{class:"contact-method fade-in"},[u("div",{class:"contact-icon",onclick:"copyToClipboard('2734664632', 'QQ号已复制到剪贴板:\\n2734664632')"}," 💬 "),u("p",{class:"contact-label"},"QQ")])],-1))])]))}},fi="/assets/logo-BBsoOzMl.png",vi={},xi={class:"footer-content"};function bi(s,t){return M(),F("footer",null,[u("div",xi,[t[2]||(t[2]=ke('<p class="copyright" data-v-b9a3d6ed>© 2024 Xnors Studio. All rights reserved.</p><div class="friend-links" data-v-b9a3d6ed><p class="friend-links-title" data-v-b9a3d6ed>友情链接</p><a href="https://xnors.github.io/xblogs/" target="_blank" class="friend-link" data-v-b9a3d6ed>异或博客/Xblogs</a><a href="https://nizhiwu.top/" target="_blank" class="friend-link" data-v-b9a3d6ed>逆之雨工作室</a><a href="https://sxxyrry.github.io/" target="_blank" class="friend-link" data-v-b9a3d6ed>23XR工作室</a></div>',2)),u("div",{class:"theme-toggle",onClick:t[0]||(t[0]=(...e)=>s.toggleTheme&&s.toggleTheme(...e))},[...t[1]||(t[1]=[u("img",{src:fi,id:"theme-icon"},null,-1)])])])])}const yi=ee(vi,[["render",bi],["__scopeId","data-v-b9a3d6ed"]]);function wi(){ge(()=>{console.log("Page effects mounted.");const s={threshold:.1,rootMargin:"0px 0px -50px 0px"},t=new IntersectionObserver(n=>{n.forEach(l=>{l.isIntersecting&&l.target.classList.add("visible")})},s);document.querySelectorAll(".fade-in").forEach(n=>{t.observe(n)});const e=n=>{const l=n.target.closest('a[href^="#"]');if(!l)return;n.preventDefault();const a=document.querySelector(l.getAttribute("href"));a&&a.scrollIntoView({behavior:"smooth",block:"start"})};document.body.addEventListener("click",e);const o=()=>{const n=document.querySelector("nav");if(!n)return;window.pageYOffset>100?(n.style.background="rgba(10, 10, 10, 0.5)",n.style.boxShadow="0 2px 20px rgba(0, 0, 0, 0.5)"):(n.style.background="rgba(10, 10, 10, 0.1)",n.style.boxShadow="none")};return window.addEventListener("scroll",o),()=>{console.log("Page effects unmounted."),t.disconnect(),document.body.removeEventListener("click",e),window.removeEventListener("scroll",o)}}),Qe(()=>{})}const Ci={__name:"Home",setup(s){return wi(),(t,e)=>(M(),F(Ee,null,[C(Xe),C(tt),C(nt),C(oi),C(gi),C(yi)],64))}};export{Ci as default};
